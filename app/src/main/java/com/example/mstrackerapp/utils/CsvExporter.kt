...
object CsvExporter {
    ...
    fun exportAndShareCsv(
        ...
        try {
            ...
            val csvFile = File(exportsDir, "MSTracker_Summary_Transactions_$timeStamp.csv")

            FileWriter(csvFile).use { writer ->
                // 1. Summary Header Section
                writer.append("=== MS TRACKER FINANCIAL SUMMARY ===\n")
            ...
            val shareIntent = Intent(Intent.ACTION_SEND).apply {
                type = "text/csv"
                putExtra(Intent.EXTRA_SUBJECT, "MS Tracker Summary & Transactions ($filterName)")
                putExtra(Intent.EXTRA_TEXT, "Attached is your MS Tracker financial summary and transactions CSV report for $filterName.")
    ...
    fun exportTransactionsToCsv(
        ...
        val exportsDir = File(context.cacheDir, "exports").apply { mkdirs() }
        val timeStamp = SimpleDateFormat("yyyyMMdd_HHmmss", Locale.ENGLISH).format(Date())
        val file = File(exportsDir, "MSTracker_Transactions_$timeStamp.csv")
    ...
    fun exportRawSmsToCsv(context: Context): File {
        val exportsDir = File(context.cacheDir, "exports").apply { mkdirs() }
        val timeStamp = SimpleDateFormat("yyyyMMdd_HHmmss", Locale.ENGLISH).format(Date())
        val file = File(exportsDir, "MSTracker_RawSms_$timeStamp.csv")
    ...
    fun shareCsvFile(context: Context, file: File, subject: String = "MS Tracker CSV Export") {
        val contentUri = FileProvider.getUriForFile(
            context,
            "${context.packageName}.fileprovider",
            file
        ...
        val shareIntent = Intent(Intent.ACTION_SEND).apply {
            ...
            putExtra(Intent.EXTRA_TEXT, "Attached CSV file from MS Tracker.")
            putExtra(Intent.EXTRA_STREAM, contentUri)
            addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
        }
