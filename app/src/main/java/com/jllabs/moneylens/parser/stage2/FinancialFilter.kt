package com.jllabs.moneylens.parser.stage2

object FinancialFilter {

    private val TRANSACTION_VERBS = Regex(
        """(?i)\b(?:debited|credited|has\s+a\s+credit\s+by|credit\s+by|withdrawn|deposited|paid|received|transferred|spent|purchase[d]?|sent|deducted|top-up|topup|top\s+up|mandate|payment|successful|trf|fastag)\b"""
    )
