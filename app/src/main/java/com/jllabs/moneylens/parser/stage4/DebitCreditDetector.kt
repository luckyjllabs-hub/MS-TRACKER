...
object DebitCreditDetector {
    ...
    private val USER_ACCOUNT_CREDITED = Regex(
        """(?i)\b(?:
            ...
            has\s+a\s+credit\s+by|
            credit\s+by\s+(?:cheque|check|transfer|neft|rtgs|imps|cash)|
    ...
    private val FASTAG_PAID = Regex(
        """(?i)\b(?:rs\.?|inr|₹)\s*[\d,]+\.?\d*.{0,40}\bpaid\s+at\b.{0,80}\bfastag\b|\bpaid\s+at\b.{0,80}\bfastag\b"""
    )
    ...
    /** Credit-card bill / payment received onto the card (reduces amount owed). */
    private val CC_PAYMENT_RECEIVED = Regex(
        """(?i)\bpayment\s+of\s+(?:inr|rs\.?|₹).{0,40}(?:has\s+been\s+)?received\s+on\s+your\s+.{0,40}credit\s*card"""
    )
    ...
    fun detect(body: String): DebitCreditResult {
        ...
        // 1. Credit Card Bill Payment Receipt -> treat as INCOME (reduces card dues)
        if (CARD_BILL_PAYMENT.containsMatchIn(body) || CC_PAYMENT_RECEIVED.containsMatchIn(body)) {
            return DebitCreditResult(TransactionType.INCOME, SmsTransactionSubType.CARD_BILL_PAYMENT)
        }
        ...
        if (INTEREST_CR.containsMatchIn(body) || lowerBody.contains("towards interest")) {
            return DebitCreditResult(TransactionType.INCOME, SmsTransactionSubType.INTEREST_CREDIT)
        }
        ...
        // FASTag toll debit
        if (FASTAG_PAID.containsMatchIn(body)) {
        ...
        if (INFO_NOTICE.containsMatchIn(body) &&
            ...
            !lowerBody.contains("credit by") &&
            !RECEIVED_IN_ACCOUNT.containsMatchIn(body) &&
            !FASTAG_PAID.containsMatchIn(body)
        ) {
        ...
        val isDebited = USER_ACCOUNT_DEBITED.containsMatchIn(body) ||
                ...
                lowerBody.contains("amt deducted") ||
                lowerBody.contains("spent using") ||
                Regex("""(?i)\bsent\s+rs\.?""").containsMatchIn(body) ||
                (lowerBody.contains("paid") && !lowerBody.contains("interest paid"))
    ...
    private fun isOwnAccountCredit(body: String, lowerBody: String): Boolean {
        ...
        if (lowerBody.contains("credited by") || lowerBody.contains("credited with") ||
            ...
            lowerBody.contains("has a credit by") || lowerBody.contains("credit by cheque") ||
            lowerBody.contains("credit by check")
        ) {
